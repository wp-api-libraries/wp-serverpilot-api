...
if ( ! class_exists( 'ServerPilotAPI' ) ) {
	...
	class ServerPilotAPI extends WpServerPilotAPIBase {
		...
		/**
		 * Whether current mode is debug or not.
		 *
		 * @var bool
		 * @access protected
		 */
		 ...
		 * @param  bool   $is_debug  (Default: false) Whether in debug mode (otherwise,
		 *                           500 errors will be masked as WP_Errors).
		 * @return void
		 */
		public function __construct( string $client_id, string $api_key, bool $is_debug = false ) {
		...
		protected function set_headers() {
			$this->args['headers'] = array(
				...
				'Authorization' => 'Basic ' . base64_encode( $this->client_id . ':' . $this->api_key ), // @codingStandardsIgnoreLine // Note, this command will be deprecated in PHP_CodeSniffer v4.0, and instead should use the phpcs: commands.
			);
		}

		/**
		 * Internal helper function for the $this->build_request( ... )->fetch() control flow.
		 *
		 * @access private
		 * @param  mixed  $route  Excepts string, but anything that's stringeable.
		 * @param  array  $args   (Default: array()) Additional arguments to pass.
		 *                        See build_request().
		 * @param  string $method (Default: 'GET') The method to run.
		 * @return object         The response.
		 */
		private function run( $route, array $args = array(), string $method = 'GET' ) {
		 ...
		 *
		 * @access public
		 * @return object A response with an array of servers under the data property.
		 */
		 ...
		 */
		public function create_system_user( string $id, string $name, string $pass = null ) {
			$args = array(
				'serverid' => $id,
				'name'     => $name,
			);
